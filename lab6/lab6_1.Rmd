...
```
### clean names with a pipe when you load the csv
```{r}
...
```{r}
fish %>% # load fish
  mutate(length_mm = length*10) %>% #multiplying length by 10 to get mm and create a new column with that info
...
mammals
```
### Changes all observations to lower case
```{r}
mammals %>%
  mutate_all(tolower)
```
...
```{r}
mammals %>% 
  mutate(across(c("order", "family"), tolower))# only changing order and family
```
...
mammals %>% 
  select(genus, species, newborn) %>%
  mutate(newborn_new = ifelse(newborn == -999.00, NA, newborn))%>% # create new variable, if sees -999.00 then puts a NA there, if it sees anything else it is going to leave it alone and keep the original value
  arrange(newborn)
```
