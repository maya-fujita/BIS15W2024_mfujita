...
```{r}
p +geom_point(size=.25)#adjust size of plot
```

We can color the points by a categorical variable.
```{r}
p+geom_point(aes(color=thermoregulation),size=1.5) #for scatter plots color= is the same as fill for bar plots
```
...
```{r}
penguins %>% 
  filter(sex!="NA") %>% #remove NA fom sex
  ggplot(aes(x=island, fill=sex))+
  geom_bar(position="dodge")+
  labs(title="Penguins by Sex on Island",
       x="Island")+
  theme(plot.title = element_text(size=rel(1.5),hjust=0.5))  
```
