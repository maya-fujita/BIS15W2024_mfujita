...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass = mean(mass, na.rm = T)) %>% 
  ggplot(aes(x=reorder(order, mean_mass), y=mean_mass))+# re order so larger animals at top and then goes to smaller animals, order x axis, by mean mass
  geom_col()+
  coord_flip()+
  scale_y_log10()
```
...
life_history %>% 
  ...
  geom_jitter(na.rm = T)# prevents oveplotting but doesnt even work for this example
```
```{r}
life_history %>% 
  ggplot(aes(x=gestation, y=wean_mass))+
...
elephants %>% 
  ...
  labs(title="Elephant Age vs. Height", #adds a title
       x="Age (years)", # name of x axis
       y="Height (cm)") # name of y axis
```
...
```{r}
elephants %>% 
  ggplot(aes(x=sex, fill=sex))+ #fill is a grouping option (default colors)
  geom_bar()
```
