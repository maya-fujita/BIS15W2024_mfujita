...
#mean sleep total of small animals 
```
...
msleep %>% 
  ...
  summarize(mean_bdwt_primate = mean(bodywt),
            min_bdwt_primate = min(bodywt),
            max_bdwt_primate = max(bodywt),
            total=n())
...
n_distinct() is a very handy way of cleanly presenting the number of distinct observations. Here we show the number of distinct genera over 100 in body weight.
```{r}
# THIS ONE IS IMPORTANT FOR EXAM!!!
...
```{r}
#means that data is not accessible for one of the observations not all of them in a single grouping
```
...
#No one has measured brain weight of whales NaN means different things though good sign that something is wrong (Not A Number - trying to take mean of 0 things so trying to divide by 0 and cant because it is not a 0)
```
