...
```{r, eval=F, echo=T}
ui <- fluidPage(
    selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm") #select input refers to drop down menu, selected= where it is going to start
)
...
ui <- fluidPage(
    ...
    plotOutput("plot", width="500px", height="400px") # provides size of the visual we are producing
)
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
      ...
      labs(x=NULL, fill="Fill Vairiable")
  })
