...
ggplot(data=homerange, mapping=aes(x=log10.mass, y=log10.hra))+
  geom_jitter() # for really really big dataset, adds random noise to distinguish points
```
...
ggplot(data=homerange, mapping=aes(x=log10.hra, y=log10.preymass))+
  geom_point(na.rm= T)+
  geom_smooth(method=lm, se=T, na.rm=T) #add a regression line
```
Positive relationship between home range and prey mass, when there are NAs it is listed on top of the plot with a warning

Removed NAs with na.m=T
...
homerange %>% 
  ...
  ggplot(aes(y=common.name, x=log10.mass))+ #notice the switch in x and y #flips
  geom_col()+
  coord_flip()# flips
```
