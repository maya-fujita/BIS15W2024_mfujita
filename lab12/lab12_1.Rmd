...
homerange %>% 
  ...
  geom_boxplot()+
  labs(title = "Mas by Taxonomic Class")
...
3. Use `group` to show the range of `log10.mass` by taxonomic.
```{r}
homerange %>% 
...
4. Use `fill` to show the range of `log10.mass` by taxonomic  with color codes.
```{r}
homerange %>% 
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "violetred4", fill = "lavenderblush2", bins=10)+#color is the outline
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(fill = "deepskyblue4", alpha = 0.3, color="black")+
...
homerange %>% 
  ggplot(aes(x=log10.hra))+
  geom_histogram(aes(y=after_stat(density)), fill = "deepskyblue4", alpha = 0.3, color="black")+
  geom_density(color="orange")+
  labs(title = "Homerange Distribution")
```
THE IDEA WITH COLOR IS TO CREATE CONSISTENCY THROUGHOUT REPORT
...
### GOING TO NEED TO DO THIS FOR MIDTERM
```{r}
homerange %>% 
...
homerange %>% 
  ...
  geom_bar(position="dodge",alpha=0.3, color="black")
```
