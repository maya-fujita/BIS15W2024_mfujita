...
```{r}
# Change the class of 'taxon' and 'order' to factors
homerange$taxon <- as.factor(homerange$taxon)
homerange$order <- as.factor(homerange$order)

# Display the levels of 'taxon'
levels(homerange$taxon)

# Display the levels of 'order'
...
taxa <- select(homerange, "taxon","common.name","class", "family", "genus", "species")
```
...
```{r}
mean(herbivore$mean.hra.m2, rm.na=T)
mean(carnivore$mean.hra.m2, rm.na=T)
```
Herbivores hace a lager mean hra.m2
```{r}
...
```{r}
owlsvariables <- select(homerange,"order","mean.mass.g", "log10.mass","family","genus")
```

```{r}
owls <- filter(owlsvariables, order == "strigiformes")
owls
```
...
```{r}
birdvariables <- select(homerange, "class","common.name", "mean.hra.m2")
bird <- filter(birdvariables, class == "aves")
```
```{r}
summary(bird$mean.hra.m2)
```
The species with the largest homerange is commonly known as caracara, which is in the falcon sub family and behaves like a falcon.
