...
sydneybeaches
```
No this data is not tidy because there are observations that should be variables, it is in the long format.
...
sydneybeaches_long_seperate %>% 
  group_by(site) %>% 
  filter(year == "2013") %>% 
  summarize(max_polluted = max(enterococci_cfu_100ml)) %>% 
  arrange(desc(max_polluted))
```
The most polluted beach was Little Bay Beach in 2013
